...
def identity(dims: Tuple[StateSpace, ...], device=None, dtype=None) -> Tensor:
    ...
    """
    if len(dims) < 2:
        raise ValueError(f"Identity tensor creation requires at least rank 2, got rank {len(dims)}!")
    matrix_dims = dims[-2:]
    rows = matrix_dims[0].size
    cols = matrix_dims[1].size
    return Tensor(data=torch.eye(rows, cols, device=device, dtype=dtype), dims=matrix_dims)
