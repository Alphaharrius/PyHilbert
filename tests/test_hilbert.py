import pytest
import types
from pyhilbert.hilbert import Mode, hilbert, HilbertSpace, MomentumSpace, operator_add, operator_sub, operator_and, operator_or, brillouin_zone
from pyhilbert.spatials import Lattice, ReciprocalLattice
...
def test_grouped_mode():
    # GroupedMode is tested implicitly if we can construct it, but let's test specifically if possible.
    # It seems GroupedMode is not exported directly in __init__.py?
    # It's in hilbert.py.
    from pyhilbert.hilbert import GroupedMode
    
    m1 = Mode(count=1, attr=FrozenDict({'id': 1}))
    m2 = Mode(count=1, attr=FrozenDict({'id': 2}))
    
    gm = GroupedMode(count=2, attr=FrozenDict({}), grouped=(m1, m2))
    
    # Update grouped mode
    gm2 = gm.update(id=lambda x: x + 10)
    # The update propagates to grouped modes?
    # GroupedMode._updated: tuple(m.update(**kwargs) for m in self.grouped)
    
    assert gm2.grouped[0].attr['id'] == 11
    assert gm2.grouped[1].attr['id'] == 12
...
def test_statespace_errors():
    ...
    # Different types addition - returns ValueError, does not raise
    res = s1 + ms
    assert isinstance(res, ValueError)
    assert "different types" in str(res)
